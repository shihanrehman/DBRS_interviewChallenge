...
4) On terminal, once you are cd'd into the folder specified in step 2) with the docker files, type -> sudo docker-compose up (Wait 5 minutes for the images to be downloaded from docker hub and copy the auth token from terminal once docker-compose finishes execution)
